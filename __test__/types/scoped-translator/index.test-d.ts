...
// Locale narrowed to en-US
{
  const translator = new ScopeTranslator<typeof messages, "en-US">(
    withMessages,
  );
  const scoped = translator.scoped();
  expectType<"hello" | "a.b.c" | "more.k" | undefined>(
...
// Locale narrowed to zh-TW
{
  const translator = new ScopeTranslator<typeof messages, "zh-TW">(
    withMessages,
  );
  const scoped = translator.scoped();
...
// Locale narrowed to en-US with locale-specific keys
{
  const translator = new ScopeTranslator<typeof messages, "en-US">(
    withMessages,
  );
  const scoped = translator.scoped("more");
